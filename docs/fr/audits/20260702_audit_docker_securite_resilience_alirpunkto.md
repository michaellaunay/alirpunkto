...
| # | Domaine | Action | Sévérité | Effort |
|---|---|---|---|---|
| S1 | Sécurité | Retirer `set -x`/echo du mot de passe LDAP ; `ldapadd -y` ; pas de `DEBUG_LDAP=true` | Critique | 30 min |
| S2 | Sécurité | Fermer le relais Postfix (audit dédié) | Critique | 1–2 h |
| S3 | Sécurité | Lier LDAP/Waitress/SMTP en loopback ou ne pas publier (aligner sur test) | Élevée | 30 min |
| S4 | Sécurité | Retirer `-d 256` de slapd en prod ; piloter `olcLogLevel` | Élevée | 15 min |
| R1 | Résilience | Limites `mem`/`cpus` sur chaque service | Élevée | 30 min |
| S5 | Sécurité | `no-new-privileges` + `cap_drop: ALL` (+ non-root) pour ldap/postfix/apache | Moyenne | 1–2 h |
| R3 | Résilience | Timer de renouvellement certbot + reload Apache | Moyenne | 1 h |
| R4 | Résilience | Sauvegardes planifiées ZODB + LDAP, test de restauration | Moyenne | 2–4 h |
| R2 | Résilience | Rotation des logs (`json-file` max-size/max-file) | Moyenne | 15 min |
| **§3** | **Bug mail** | **`restart: unless-stopped` en test + healthcheck fonctionnel + entrypoint fiabilisé** | — | **1 h** |
| S6–S9, R5–R9 | Mixte | Credentials hors image, en-têtes/TLS Apache, `chmod 600 .env`, segmentation réseau, nettoyage prod, `internal:true`/commentaire | Faible | variable |

## Fichiers à fournir pour compléter l'audit
`docker/etc/postfix/main.cf`, `docker/etc/opendkim.conf`, `docker/etc/apache2/sites-available/alirpunkto.conf.template` (vhost **de prod**) — nécessaires pour valider définitivement le durcissement TLS/en-têtes Apache, le mode OpenDKIM, et d'éventuels réglages Postfix additionnels non couverts par les `postconf -e` des scripts.
