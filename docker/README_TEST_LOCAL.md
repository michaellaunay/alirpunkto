...
> Test services now use `restart: unless-stopped` (like production, **R5**), so a
> transient exit self-heals instead of leaving a dead container. Postfix also
> has a functional SMTP healthcheck (it opens a connection and expects the `220`
> greeting, **R8**), so `depends_on: service_healthy` only releases Pyramid once
> Postfix actually answers.
...
- Postfix runs in sink mode and does not relay messages (it no longer runs
  OpenDKIM — the sink discards mail, so there is nothing to sign — and Postfix
  runs as PID 1 for a simpler, self-healing container, §3);
- LDAP and Pyramid communicate only through the Docker network;
- published ports are bound to `127.0.0.1`.

The test network is a normal bridge (**not** `internal`, despite what an older
comment suggested — R9). The Pyramid test container runs
`pip install -e ".[testing]"` at **startup** when `INSTALL_EXTRAS_TESTING=true`
(the default), so it needs Internet access at runtime. Declaring the network
`internal: true` would break that install unless you also provide a local PyPI
mirror/proxy:
...
networks:
  alirpunkto-test-net:
    ...
    driver: bridge
    # internal: true   # only if extras are pre-baked (see below) or a PyPI mirror exists
```

To run fully offline, build the images while packages are available, then start
with `INSTALL_EXTRAS_TESTING=false` (extras already present) so nothing is
fetched at runtime.
