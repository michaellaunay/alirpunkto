...
# Eighth audit pass (2026-08-02, §4/§5): nothing user-provided rides
# argv — the command line carries the two file paths only, and every
# value crosses as NUL-delimited NAME=VALUE records on the generator's
# standard input. Tenth audit pass (§10/§11): the record emitter moved
# to docker/ldif_records.sh, the ONE copy of the transport contract
# shared with docker/init_test.sh and the smoke workflow — changing
# the generator's interface now changes every caller together. A pipe
# is hidden from ordinary process inspection (unlike argv); root or an
# authorised tracer can still read a process's memory. The generator
# refuses to run if a required field (a password above all) is missing
# or empty, and hashes the passwords itself.
ADMIN_UUID="${LDAP_ADMIN_OID}"
